...
function makeFyCgtSummary(
  overrides: Partial<FyCgtSummary> & Pick<FyCgtSummary, 'financialYear'>,
): FyCgtSummary {
  return {
    financialYear: overrides.financialYear,
    lots: overrides.lots ?? [],
...
describe('toEssIncomeByFyBars', () => {
  ...
  it('splits standard vs 30-day income per FY (AUD)', () => {
    ...
    expect(result).toEqual([{ fy: '2022-23', standard: 6000, thirtyDay: 4000 }])
  })
  ...
  it('splits standard vs 30-day income per FY (USD)', () => {
    ...
    expect(result).toEqual([{ fy: '2022-23', standard: 4500, thirtyDay: 1500 }])
  })
})
