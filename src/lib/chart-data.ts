...
export function toSharePriceLine(releaseIncomes: ReleaseEssIncome[]): SharePricePoint[] {
  return sortedByDate(releaseIncomes).map((r) => ({
    date: toDateKey(r.releaseDate),
...
export function toEssIncomeByFyBars(
  ...
  return fyEssIncomes.map((fy) => {
    ...
    const thirtyDay = sumAud(fy.releases.flatMap((r) => r.thirtyDayLots.map((l) => l.essIncomeAud)))
    return {
      fy: fy.financialYear,
      standard: standard as number,
      thirtyDay: thirtyDay as number,
...
export function toCgtByFyBars(fyCgtSummaries: FyCgtSummary[], currency: 'USD' | 'AUD'): CgtFyBar[] {
...
export function toCumulativeEssIncome(
  ...
  return sorted.map((r) => {
    cumulative +=
      currency === 'USD' ? (r.totalEssIncomeUsd as number) : (r.totalEssIncomeAud as number)
