...
function awardCsv(count: number): string {
  const headers = [
    'Award Summary',
...
function vestingCsv(grantCount: number, entriesPerGrant: number): string {
  const headers = [
...
function releasesCsv(count: number): string {
  const headers = [
    'RSU Releases',
...
function salesCsv(count: number): string {
  const headers = [
    'Sales - Long Shares',
