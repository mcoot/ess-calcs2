...
function makeLot(overrides: Partial<SaleLot> & Pick<SaleLot, "saleDate" | "originalAcquisitionDate" | "soldWithin30Days" | "sharesSold" | "saleProceeds" | "costBasis">): SaleLot {
  return {
...
const CGT_RATES: ForexRate[] = [
  { date: d(2019, 2, 18), audToUsd: 0.7200 },  // Case 1 acq
  { date: d(2021, 8, 3), audToUsd: 0.7400 },   // Case 1 sale
  { date: d(2021, 8, 18), audToUsd: 0.7300 },  // Case 2 acq
  { date: d(2022, 6, 1), audToUsd: 0.7500 },   // Cases 13,15 long-term acq
  { date: d(2022, 11, 8), audToUsd: 0.6500 },  // Case 2 sale
  { date: d(2023, 1, 2), audToUsd: 0.6800 },   // Cases 5,6 acq
  { date: d(2023, 6, 1), audToUsd: 0.8000 },   // Case 16 acq
  { date: d(2024, 1, 2), audToUsd: 0.6900 },   // Cases 5,13-short acq; case 5 sale
  { date: d(2024, 1, 3), audToUsd: 0.6910 },   // Case 6 sale
  { date: d(2024, 3, 1), audToUsd: 0.6000 },   // Cases 4,7,8,10,12,14,15 acq
  { date: d(2024, 5, 25), audToUsd: 0.6050 },  // Case 13 lot C acq (30-day)
  { date: d(2024, 6, 3), audToUsd: 0.6200 },   // Multiple cases: sale date
  { date: d(2024, 9, 2), audToUsd: 0.5000 },   // Case 16 sale
  { date: d(2025, 3, 14), audToUsd: 0.6300 },  // Case 9 fallback (Fri for Sat acq)
  { date: d(2025, 3, 18), audToUsd: 0.6310 },  // Case 9 fallback (gap day sale)
  { date: d(2025, 4, 1), audToUsd: 0.6400 },   // Case 14 sale2 (long-term)
];
...
describe("CgtService — calculateByLot", () => {
  ...
  it("Case 2: capital loss — no discount on losses", () => {
    const lot = makeLot({
      ...
      soldWithin30Days: false,
      sharesSold: 30,
      costBasis: usd(10131.90),
      costBasisPerShare: usd(337.73),
      saleProceeds: usd(3618.00),
      salePricePerShare: usd(120.60),
  ...
  it("Case 7: two different forex rates — acquisition date vs sale date", () => {
    ...
    expect(result.acquisitionForexRate).toBe(0.6000);
    expect(result.acquisitionForexDate).toEqual(d(2024, 3, 1));
    expect(result.saleForexRate).toBe(0.6200);
  ...
  it("Case 8: net proceeds deducts brokerage and fees", () => {
    ...
    expect(result.brokerageUsd).toBeCloseTo(45.50, 2);
  ...
  it("Case 9: forex weekend/gap fallback on both acquisition and sale dates", () => {
    ...
    expect(result.acquisitionForexRate).toBe(0.6300);
    expect(result.acquisitionForexDate).toEqual(d(2025, 3, 14));
    expect(result.saleForexRate).toBe(0.6310);
  ...
  it("Case 12: audit trail — all source fields populated", () => {
    const lot = makeLot({
      ...
      supplementalTransactionFee: usd(0.50),
    });
    ...
    expect(result.feesUsd).toBeCloseTo(0.50, 2);
  });
  ...
  it("Case 13: same sale date, mixed holding periods — independent per-lot calc", () => {
    ...
    expect(lt.costBasisAud).toBeCloseTo(2000.00, 2);
    ...
    expect(st.netProceedsAud).toBeCloseTo(1612.90, 2);
  ...
  it("Case 14: same acquisition date, different sale dates — different holding/forex", () => {
    ...
    expect(early.acquisitionForexRate).toBe(0.6000);
    expect(late.acquisitionForexRate).toBe(0.6000);

    // Different sale rates
    expect(early.saleForexRate).toBe(0.6200);
    expect(late.saleForexRate).toBe(0.6400);
  ...
  it("Case 15: gain on short-term, loss on long-term in same sale", () => {
    ...
    expect(sg.capitalGainLossAud).toBeCloseTo(295.70, 2);
  ...
  it("Case 16: currency movement creates AUD gain despite USD loss", () => {
    ...
    expect(result.costBasisAud).toBeCloseTo(1250.00, 2);
    expect(result.netProceedsAud).toBeCloseTo(1800.00, 2);

    // AUD gain = 1800 - 1250 = 550 (positive gain despite USD loss!)
    expect(result.capitalGainLossAud).toBeCloseTo(550.00, 2);
    expect(result.isLongTerm).toBe(true);
    expect(result.isDiscountEligible).toBe(true);
  });
});
...
describe("CgtService — aggregateByFy", () => {
  ...
  it("Case 1: all long-term gains, no losses — 50% discount applied", () => {
    const lots = [
      makeLotCgt({ capitalGainLossAud: aud(1000), isLongTerm: true, financialYear: "2023-24", lotNumber: 1 }),
  ...
  it("Case 2: all short-term gains, no losses — no discount", () => {
    const lots = [
      makeLotCgt({ capitalGainLossAud: aud(600), isLongTerm: false, financialYear: "2023-24", lotNumber: 1 }),
      makeLotCgt({ capitalGainLossAud: aud(400), isLongTerm: false, financialYear: "2023-24", lotNumber: 2 }),
    ];
  ...
  it("Case 4: losses exceed short-term, remainder applied to long-term", () => {
    ...
    const lots = [
      makeLotCgt({ capitalGainLossAud: aud(500), isLongTerm: false, financialYear: "2023-24", lotNumber: 1 }),
      makeLotCgt({ capitalGainLossAud: aud(3000), isLongTerm: true, financialYear: "2023-24", lotNumber: 2 }),
  ...
  it("Case 5: losses exceed all gains — net capital loss", () => {
    ...
    const lots = [
      makeLotCgt({ capitalGainLossAud: aud(200), isLongTerm: false, financialYear: "2023-24", lotNumber: 1 }),
      makeLotCgt({ capitalGainLossAud: aud(300), isLongTerm: true, financialYear: "2023-24", lotNumber: 2 }),
      makeLotCgt({ capitalGainLossAud: aud(-1000), isLongTerm: true, financialYear: "2023-24", lotNumber: 3 }),
    ];
  ...
  it("Case 6: only losses, no gains", () => {
    const lots = [
      makeLotCgt({ capitalGainLossAud: aud(-3000), isLongTerm: true, financialYear: "2023-24", lotNumber: 1 }),
      makeLotCgt({ capitalGainLossAud: aud(-500), isLongTerm: false, financialYear: "2023-24", lotNumber: 2 }),
    ];
  ...
  it("Case 7: mixed short-term and long-term losses both contribute to total", () => {
    ...
    const lots = [
      makeLotCgt({ capitalGainLossAud: aud(800), isLongTerm: false, financialYear: "2023-24", lotNumber: 1 }),
      makeLotCgt({ capitalGainLossAud: aud(-400), isLongTerm: false, financialYear: "2023-24", lotNumber: 2 }),
      makeLotCgt({ capitalGainLossAud: aud(-600), isLongTerm: true, financialYear: "2023-24", lotNumber: 3 }),
    ];
  ...
  it("Case 8: multiple FYs — each aggregated independently", () => {
    const lots = [
      ...
      makeLotCgt({ capitalGainLossAud: aud(2000), isLongTerm: true, financialYear: "2022-23", lotNumber: 1 }),
      // FY 2023-24: gain of 500 (short-term) + loss of 300
      makeLotCgt({ capitalGainLossAud: aud(500), isLongTerm: false, financialYear: "2023-24", lotNumber: 2 }),
      makeLotCgt({ capitalGainLossAud: aud(-300), isLongTerm: true, financialYear: "2023-24", lotNumber: 3 }),
    ];
