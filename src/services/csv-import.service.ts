...
function parseByType(type: DataType, csvText: string) {
  switch (type) {
    case 'awards':
      return parseAwardSummary(csvText)
    case 'vestingSchedule':
      return parseVestingSchedule(csvText)
    case 'releases':
      return parseRsuReleases(csvText)
    case 'saleLots':
      return parseSales(csvText)
    default:
      return { ok: false as const, error: `Unsupported type: ${type}` }
  }
}
