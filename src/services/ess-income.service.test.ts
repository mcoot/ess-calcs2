...
const TEST_RATES: ForexRate[] = [
  ...
  { date: d(2020, 2, 18), audToUsd: 0.67 },

  // 2023: covers Case 2 (13-Feb-2023 is a Monday)
  { date: d(2023, 2, 13), audToUsd: 0.6921 },

  // Two different dates for Case 3
  { date: d(2023, 5, 15), audToUsd: 0.665 },
  { date: d(2023, 11, 13), audToUsd: 0.64 },

  // FY boundary: covers Cases 4 & 5
  { date: d(2024, 2, 18), audToUsd: 0.65 },
  // 28-Jun is a Friday, 30-Jun is a Sunday (no rate), 1-Jul is Monday
  { date: d(2024, 6, 28), audToUsd: 0.66 },
  { date: d(2024, 7, 1), audToUsd: 0.661 },
  { date: d(2024, 8, 13), audToUsd: 0.655 },

  // Case 6: weekend fallback — 15-Mar-2025 is Saturday, 14-Mar is Friday
  { date: d(2025, 3, 14), audToUsd: 0.63 },

  // Case 7: gap in data — 19-Mar-2025 is Wednesday with no rate, 18-Mar has one
  { date: d(2025, 3, 18), audToUsd: 0.631 },

  // Cases 9-11
  { date: d(2025, 5, 13), audToUsd: 0.645 },

  // 30-day rule test rates
  { date: d(2024, 3, 4), audToUsd: 0.5 }, // vest date (Monday)
  { date: d(2024, 3, 11), audToUsd: 0.625 }, // sale date case 1
  { date: d(2024, 3, 8), audToUsd: 0.7 }, // sale date case 2
  { date: d(2024, 4, 1), audToUsd: 0.8 }, // multi-lot vest date
  { date: d(2024, 4, 3), audToUsd: 0.64 }, // multi-lot sale 1
  { date: d(2024, 4, 5), audToUsd: 0.5 }, // multi-lot sale 2
  { date: d(2024, 4, 8), audToUsd: 0.6 }, // multi-lot sale 3
  { date: d(2024, 7, 3), audToUsd: 0.662 }, // cross-FY sale date
  { date: d(2023, 2, 24), audToUsd: 0.68 }, // spec test case 2 sale date
]
...
describe('EssIncomeService — standard case (no 30-day rule)', () => {
  ...
  it('Case 1: simple release, no sale lots', () => {
    ...
    expect(result.standardIncomeUsd).toBeCloseTo(4616.4, 2)
  ...
  it('Case 3: multiple releases produce independent results with own forex rates', () => {
    ...
    expect(r1.standardIncomeUsd).toBeCloseTo(4000.0, 2)
    expect(r1.standardIncomeAud).toBeCloseTo(6015.04, 2)
    expect(r1.standardForexRate).toBe(0.665)

    // Release 2: 10 × 180 = 1800 USD, / 0.6400 = 2812.50
    const r2 = results.find((r) => r.releaseRef === 'REL-B')!
    expect(r2.standardIncomeUsd).toBeCloseTo(1800.0, 2)
    expect(r2.standardIncomeAud).toBeCloseTo(2812.5, 2)
    expect(r2.standardForexRate).toBe(0.64)
  })
  ...
  it('Case 6: vest date on weekend — forex falls back to prior business day', () => {
    ...
    expect(result.standardForexRate).toBe(0.63)
    expect(result.standardForexDate).toEqual(d(2025, 3, 14))
    // AUD: 2000 / 0.63 = 3174.603... → 3174.60
    expect(result.standardIncomeAud).toBeCloseTo(3174.6, 2)
  })
  ...
  it('Case 7: vest date on gap day — forex falls back to prior available date', () => {
    ...
    expect(result.standardForexRate).toBe(0.631)
...
describe('EssIncomeService — 30-day rule', () => {
  ...
  it('Case 1: mixed release — some shares 30-day, remainder standard', () => {
    ...
    expect(result.standardIncomeAud).toBeCloseTo(2400.0, 2)
    ...
    expect(result.thirtyDayLots[0].essIncomeAud).toBeCloseTo(1440.0, 2)

    // Total = 2400 + 1440 = 3840.00
    expect(result.totalEssIncomeAud).toBeCloseTo(3840.0, 2)
    // USD total = standardIncomeUsd (1200) + saleProceedsUsd (900) = 2100
    expect(result.totalEssIncomeUsd).toBeCloseTo(2100.0, 2)
  })
  ...
  it('Case 2: all shares sold within 30 days — zero standard shares', () => {
    ...
    expect(result.thirtyDayLots).toHaveLength(1)
    expect(result.thirtyDayLots[0].essIncomeAud).toBeCloseTo(3000.0, 2)
    expect(result.totalEssIncomeAud).toBeCloseTo(3000.0, 2)
  ...
  it('Case 3: multiple 30-day lots from one release, each with own forex rate', () => {
    ...
    expect(l1.essIncomeAud).toBeCloseTo(2500.0, 2)
    expect(l1.forexRate).toBe(0.64)

    // Lot 2: 1000 / 0.50 = 2000.00
    const l2 = result.thirtyDayLots.find((l) => l.saleLotRef === 'WRC-M2')!
    expect(l2.essIncomeAud).toBeCloseTo(2000.0, 2)
    expect(l2.forexRate).toBe(0.5)
    ...
    expect(result.totalEssIncomeUsd).toBeCloseTo(4000.0, 2)
  })
  ...
  it('Case 4: 30-day lot forex uses sale date, not vest date', () => {
    ...
    expect(result.thirtyDayLots[0].forexRate).toBe(0.625)
    expect(result.thirtyDayLots[0].forexDate).toEqual(d(2024, 3, 11))
    // AUD = 1000 / 0.625 = 1600, NOT 1000 / 0.5 = 2000
    expect(result.thirtyDayLots[0].essIncomeAud).toBeCloseTo(1600.0, 2)
  })
  ...
  it('Case 6: 30-day lot audit trail fields fully populated', () => {
    ...
    expect(lotResult.forexRate).toBe(0.625)
    expect(lotResult.forexDate).toEqual(d(2024, 3, 11))
    // 600 / 0.625 = 960.00
    expect(lotResult.essIncomeAud).toBeCloseTo(960.0, 2)
...
function makeReleaseIncome(
  overrides: Partial<ReleaseEssIncome> &
    Pick<
      ReleaseEssIncome,
      'releaseRef' | 'financialYear' | 'standardIncomeAud' | 'totalEssIncomeAud'
    >,
): ReleaseEssIncome {
  ...
  return {
    ...
    totalEssIncomeUsd:
      overrides.totalEssIncomeUsd ??
      usd(
        (standardIncomeUsd as number) +
          thirtyDayLots.reduce((sum, l) => sum + (l.saleProceedsUsd as number), 0),
      ),
