...
export interface ReportService {
  ...
  availableFinancialYears(releases: RsuRelease[], saleLots: SaleLot[]): string[]
}
...
export function createReportService(): ReportService {
  ...
  function buildCgtRows(fyLots: SaleLotCgt[], grantNames: Map<number, string>): CgtReportRow[] {
  ...
  function generateFyReport(
    ...
    const essIncomeTotalAud = fyEss ? aud(roundTo2dp(fyEss.totalEssIncomeAud as number)) : aud(0)
    ...
    const thirtyDaySummaryRows = fyEss ? buildThirtyDaySummary(fyEss.releases, grantNames) : []
  ...
  function availableFinancialYears(releases: RsuRelease[], saleLots: SaleLot[]): string[] {
