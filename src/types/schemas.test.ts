...
describe('AwardSchema', () => {
  ...
  it('fails when sharesGranted is a string', () => {
    expect(AwardSchema.safeParse({ ...validAward(), sharesGranted: 'ten' }).success).toBe(false)
  })

  it('fails when grantNumber is 0', () => {
    expect(AwardSchema.safeParse({ ...validAward(), grantNumber: 0 }).success).toBe(false)
  })

  it('fails when grantNumber is negative', () => {
    expect(AwardSchema.safeParse({ ...validAward(), grantNumber: -1 }).success).toBe(false)
  })

  it('fails when grantNumber is non-integer', () => {
    expect(AwardSchema.safeParse({ ...validAward(), grantNumber: 1.5 }).success).toBe(false)
  })

  it('fails when grantType is empty', () => {
    expect(AwardSchema.safeParse({ ...validAward(), grantType: '' }).success).toBe(false)
  })

  it('fails when grantName is empty', () => {
    expect(AwardSchema.safeParse({ ...validAward(), grantName: '' }).success).toBe(false)
  })

  it('fails when grantReason is empty', () => {
    expect(AwardSchema.safeParse({ ...validAward(), grantReason: '' }).success).toBe(false)
  })

  it('allows conversionPrice of zero', () => {
    expect(AwardSchema.safeParse({ ...validAward(), conversionPrice: 0 }).success).toBe(true)
  })

  it('fails when conversionPrice is negative', () => {
    expect(AwardSchema.safeParse({ ...validAward(), conversionPrice: -1 }).success).toBe(false)
  })

  it('fails when sharesGranted is 0', () => {
    expect(AwardSchema.safeParse({ ...validAward(), sharesGranted: 0 }).success).toBe(false)
  })

  it('fails when sharesGranted is non-integer', () => {
    expect(AwardSchema.safeParse({ ...validAward(), sharesGranted: 10.5 }).success).toBe(false)
  })
})
...
describe('RsuReleaseSchema', () => {
  ...
  it('fails when grantNumber is 0', () => {
    expect(RsuReleaseSchema.safeParse({ ...validRsuRelease(), grantNumber: 0 }).success).toBe(false)
  })

  it('fails when grantName is empty', () => {
    expect(RsuReleaseSchema.safeParse({ ...validRsuRelease(), grantName: '' }).success).toBe(false)
  })

  it('fails when grantReason is empty', () => {
    expect(RsuReleaseSchema.safeParse({ ...validRsuRelease(), grantReason: '' }).success).toBe(
      false,
    )
  })

  it('fails when sharesVested is 0', () => {
    expect(RsuReleaseSchema.safeParse({ ...validRsuRelease(), sharesVested: 0 }).success).toBe(
      false,
    )
  })
  ...
  it('allows sharesHeld of 0', () => {
    expect(RsuReleaseSchema.safeParse({ ...validRsuRelease(), sharesHeld: 0 }).success).toBe(true)
  })

  it('fails when sharesHeld is negative', () => {
    expect(RsuReleaseSchema.safeParse({ ...validRsuRelease(), sharesHeld: -1 }).success).toBe(false)
  })

  it('fails when valueUsd is 0', () => {
    expect(RsuReleaseSchema.safeParse({ ...validRsuRelease(), valueUsd: 0 }).success).toBe(false)
  })

  it('fails when fmvPerShare is 0', () => {
    expect(RsuReleaseSchema.safeParse({ ...validRsuRelease(), fmvPerShare: 0 }).success).toBe(false)
  })
...
describe('SaleLotSchema', () => {
  ...
  it('fails when grantNumber is 0', () => {
    expect(SaleLotSchema.safeParse({ ...validSaleLot(), grantNumber: 0 }).success).toBe(false)
  })

  it('fails when grantName is empty', () => {
    expect(SaleLotSchema.safeParse({ ...validSaleLot(), grantName: '' }).success).toBe(false)
  })

  it('fails when lotNumber is 0', () => {
    expect(SaleLotSchema.safeParse({ ...validSaleLot(), lotNumber: 0 }).success).toBe(false)
  })

  it('fails when saleType is empty', () => {
    expect(SaleLotSchema.safeParse({ ...validSaleLot(), saleType: '' }).success).toBe(false)
  })

  it('allows costBasisPerShare of 0', () => {
    expect(SaleLotSchema.safeParse({ ...validSaleLot(), costBasisPerShare: 0 }).success).toBe(true)
  })

  it('fails when costBasisPerShare is negative', () => {
    expect(SaleLotSchema.safeParse({ ...validSaleLot(), costBasisPerShare: -1 }).success).toBe(
      false,
    )
  })

  it('allows costBasis of 0', () => {
    expect(SaleLotSchema.safeParse({ ...validSaleLot(), costBasis: 0 }).success).toBe(true)
  })

  it('fails when costBasis is negative', () => {
    expect(SaleLotSchema.safeParse({ ...validSaleLot(), costBasis: -1 }).success).toBe(false)
  })

  it('fails when sharesSold is 0', () => {
    expect(SaleLotSchema.safeParse({ ...validSaleLot(), sharesSold: 0 }).success).toBe(false)
  })

  it('fails when sharesSold is non-integer', () => {
    expect(SaleLotSchema.safeParse({ ...validSaleLot(), sharesSold: 5.5 }).success).toBe(false)
  })

  it('allows saleProceeds of 0', () => {
    expect(SaleLotSchema.safeParse({ ...validSaleLot(), saleProceeds: 0 }).success).toBe(true)
  })

  it('fails when saleProceeds is negative', () => {
    expect(SaleLotSchema.safeParse({ ...validSaleLot(), saleProceeds: -1 }).success).toBe(false)
  })

  it('fails when salePricePerShare is 0', () => {
    expect(SaleLotSchema.safeParse({ ...validSaleLot(), salePricePerShare: 0 }).success).toBe(false)
  })

  it('allows brokerageCommission of 0', () => {
    expect(SaleLotSchema.safeParse({ ...validSaleLot(), brokerageCommission: 0 }).success).toBe(
      true,
    )
  })

  it('fails when brokerageCommission is negative', () => {
    expect(SaleLotSchema.safeParse({ ...validSaleLot(), brokerageCommission: -1 }).success).toBe(
      false,
    )
  })
...
describe('ForexRateSchema', () => {
  ...
  it('fails when audToUsd is 0', () => {
    expect(ForexRateSchema.safeParse({ ...validForexRate(), audToUsd: 0 }).success).toBe(false)
  })

  it('fails when audToUsd is negative', () => {
    expect(ForexRateSchema.safeParse({ ...validForexRate(), audToUsd: -0.5 }).success).toBe(false)
  })
})
...
describe('AppConfigSchema', () => {
  ...
  it('fails when displayCurrency is invalid', () => {
    expect(AppConfigSchema.safeParse({ displayCurrency: 'EUR' }).success).toBe(false)
  })
